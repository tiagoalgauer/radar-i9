...
def montar_digest(mencoes, link_painel, intervalo_dias, nome="Radar i9+"):
    total = len(mencoes)
    top = mencoes[:TOP_DIGEST]
    restantes = total - len(top)
    marcas = sum(1 for m in mencoes if m.marca)
    assunto = f"📰 {nome} — Digest: {total} menções nos últimos {intervalo_dias} dias" + (f" ({marcas} da marca)" if marcas else "")
    if total == 0:
        texto = f"Nenhuma menção nova nos últimos {intervalo_dias} dias. O robô continua vigiando.\nPainel: {link_painel}\n"
        html_ = f"<h2>{html.escape(nome)} — Digest</h2><p>Nenhuma menção nova nos últimos {intervalo_dias} dias. O robô continua vigiando.</p><p><a href=\"{html.escape(link_painel)}\">Abrir o Painel</a></p>"
        return assunto, texto, html_
    rodape = f"\n… e mais {restantes} no Painel: {link_painel}\n" if restantes else f"\nPainel completo: {link_painel}\n"
    texto = f"{total} menções nos últimos {intervalo_dias} dias (menções à marca primeiro, depois por relevância):\n\n" + "\n".join(_item_texto(m) for m in top) + rodape
    ...
    html_ = f"<h2>{html.escape(nome)} — Digest</h2><p>{total} menções nos últimos {intervalo_dias} dias (marca primeiro, depois por relevância):</p>" + "".join(_item_html(m) for m in top) + rodape_html
    return assunto, texto, html_
